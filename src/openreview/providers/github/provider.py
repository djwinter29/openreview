...
from openreview.providers.github.sync import (
...
class GitHubProvider:
    ...
    def apply(self, pr_id: int, actions: list[Any], *, dry_run: bool = False) -> SyncSummary:
        created = sum(1 for a in actions if a.kind == "create_review_comment")
        updated = sum(1 for a in actions if a.kind == "update_review_comment")
        closed = sum(1 for a in actions if a.kind == "close_review_comment")
        if dry_run:
