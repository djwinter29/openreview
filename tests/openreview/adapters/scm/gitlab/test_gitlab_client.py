...
def test_gitlab_client_methods(monkeypatch):
    client = GitLabClient(project_id="group%2Frepo", token="t", base_url="https://gitlab.example/api/v4")
    fake = FakeHttp()
    monkeypatch.setattr(client, "_client", lambda: fake)

    notes = client.get_mr_notes(7)
    assert notes[0]["id"] == 1

    created = client.create_mr_note(7, "hello")
    assert created["body"] == "hello"

    updated = client.update_mr_note(7, 3, "world")
