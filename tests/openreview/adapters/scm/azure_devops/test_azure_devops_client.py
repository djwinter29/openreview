...
def test_client_endpoints(monkeypatch) -> None:
    fake = FakeHttpClient()
    client = AzureDevOpsClient("org", "proj", "repo", "pat")
    monkeypatch.setattr(client, "_client", lambda: fake)

    threads = client.get_pull_request_threads(7)
    assert threads == [{"id": 11}]

    paths = client.get_changed_files_latest_iteration(7)
    assert paths == ["/a.c", "/b.c"]

    client.create_thread(7, {"x": 1})
    client.update_thread(7, 9, {"status": 1})
    client.create_comment(7, 9, "hello")
