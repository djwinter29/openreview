...
def test_create_update_close_flow() -> None:
    ...
    assert len(actions) == 1 and actions[0].kind == "create_comment"

    # update
    existing = [{"id": 10, "body": "<!-- openreview:fingerprint=f1 -->\nold"}]
    actions = plan_github_sync([rf("f1", "new message")], existing)
    assert any(a.kind == "update_comment" for a in actions)

    # close missing
    existing = [{"id": 10, "body": "<!-- openreview:fingerprint=f1 -->\nold"}]
    actions = plan_github_sync([], existing)
    assert len(actions) == 1 and actions[0].kind == "close_comment"
